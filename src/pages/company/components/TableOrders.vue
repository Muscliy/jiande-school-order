...
import { OrderStatusStrMap, OrderStatus } from "@/helpers/constants";
...
export default {
  ...
  methods: {
    ...
    handleClickItem(event) {
      ...
      if (
        item.orderStatus === OrderStatus.WaitingDispatch ||
        item.orderStatus === OrderStatus.ChargeBack
      ) {
        uni.navigateTo({
          url: `/pages/dispatch/index?orderId=${item.orderId}`,
        });
      } else {
        uni.navigateTo({
